...
export function initOrdersManagement({ supabaseClient, ui, helpers, state }) {
  ...
  function renderClinicianScoresPanel(scoresObj) {
    ...
    const rowHtml = (key) => {
      ...
      const rawLabel = String(key || "").split("_").join(" ");
      const title = tipTitleCase(rawLabel);
      const label = escapeHtml(rawLabel);
      const tipText = DIM_TIPS[key] || "";
      return `
        <div class="scoreRow" data-score-key="${escapeHtml(key)}">
          <div class="scoreLabel"><span class="tip-trigger" data-tip-title="${escapeHtml(title)}" data-tip="${escapeHtml(tipText)}">${label}</span></div>
          <div>
